...
schema = json.load(open('io.chemx.digital_material_passport/0.8.1/gen/DigitalMaterialPassport-schema.json', encoding='utf-8'))
template = json.load(open('io.chemx.digital_material_passport/0.8.1/gen/DigitalMaterialPassport.json', encoding='utf-8'))
...
Three things have to say the same word, and they are all derived from the
catalogue row rather than chosen here:

| | |
|---|---|
| namespace | `io.chemx.digital_material_passport` — this directory |
| aspect | `DigitalMaterialPassport` — the fragment after `#` in the URN |
| URN | `urn:samm:io.chemx.digital_material_passport:0.8.1#DigitalMaterialPassport` |

The versioning service fetches `{namespace}/{version}/gen/{aspect}-schema.json`
and `{aspect}.json`, and publishes every artefact as
`{aspect}-{version}-...`. So renaming either half means renaming this directory
or these files in the same commit, or every fetch 404s.

Spelled out rather than abbreviated to `dmp`: the namespace follows the
catalogue's `io.<owner>.<model>` convention, and an abbreviation would have made
the namespace, the aspect and the directory disagree.
...
`io.chemx.digital_material_passport/0.8.1/gen/DigitalMaterialPassport-schema.json` here does **nothing** once 0.8.1 has
...
`io.chemx.digital_material_passport/<version>/{metadata.json, gen/DigitalMaterialPassport-schema.json, gen/DigitalMaterialPassport.json}`;
...
`source_repo="chemx-official"` on the `io.chemx.digital_material_passport` row in
